...
export async function migrateContract(RPC_ENDPOINT: string, wallet: DirectSecp256k1HdWallet, contractAddress: string, newCodeId: number, migrateMsg: object, memo: string) {
  const fee = calculateFee(300000, "0.1usei");
  const [firstAccount] = await wallet.getAccounts();
  const signCosmWasmClient = await getSigningCosmWasmClient(RPC_ENDPOINT, wallet);
